...
begin
  ...
  Else
    Begin
      ...
      If ((qrClientes.State = dsBrowse) Or (qrClientes.State = dsEdit)) Then //and
//         (Length(trim(cxDBMaskEdit1.Text))=14)  Then
        Begin
          qfind := DM.NewQuery;
          DM.qrParametro.close;
          DM.qrParametro.open;
          ...
          If (qrClientesGrupoCliente.Value = DM.qrParametroGrupoAcc.Value) Or
            ...
            Begin
             qfind.Close;
             qfind.SQL.Text := ' SELECT NOMBRES,APELLIDO1,ISNULL(APELLIDO2,'+#39+#39+') AS APELLIDO2,FECHA_NAC,SEXO,DIRECTORIO,ARCHIVO,COD_SANGRE,(SELECT RUTA_FOTO FROM dbo.ULTIMA_SINCRONIA ) AS Ruta_Foto FROM Padron.dbo.PDRONCF1 '+
                               ' WHERE MUN_CED ='+#39+Copy(qrClientesIdentificacion.Value,1,3)+#39+
                               ' AND   SEQ_CED ='+#39+copy(qrClientesIdentificacion.Value,4,7)+#39+
                               ' AND   VER_CED ='+#39+copy(qrClientesIdentificacion.Value,11,1)+#39;
             qfind.Open;
             If (qfind.RecordCount > 0) Then
             begin
               cxDBMaskEdit1.EditText := qrClientesIDENTIFICACION.Value;
             EdNombre.Text := qfind.FieldByName('Nombres').AsString+' '+
               ...
               if FileExists(qfind.FieldByName('Ruta_Foto').AsString+qfind.FieldByName('Directorio').AsString+'\'+qfind.FieldByName('archivo').AsString) then
                 imgFoto.Picture.LoadFromFile(qfind.FieldByName('Ruta_Foto').AsString+qfind.FieldByName('Directorio').AsString+'\'+qfind.FieldByName('archivo').AsString)
               else
                 imgFoto.Picture := Nil;
             end
             Else
...
//  If (qrClientes.State = dsBrowse) and (Length(qrClientesIdentificacion.Value)=11)
//     and (pcDatos.ActivePageIndex = 1) then
//  If (qrClientes.State = dsBrowse) and (Length(qrClientesIdentificacion.Value)=11) and (buscar) then
    ...
    Begin
      ...
      If (qrClientesGrupoCliente.Value = DM.qrParametroGrupoAcc.Value) Or
        ...
        Begin
         qfind.Close;
         qfind.SQL.Text := ' SELECT NOMBRES,APELLIDO1,ISNULL(APELLIDO2,'+#39+#39+') AS APELLIDO2,FECHA_NAC,SEXO,DIRECTORIO,ARCHIVO,COD_SANGRE,(SELECT RUTA_FOTO FROM dbo.ULTIMA_SINCRONIA ) AS Ruta_Foto FROM Padron.dbo.PDRONCF1 '+
                           ' WHERE MUN_CED ='+#39+Copy(cxDBMaskEdit1.Text,1,3)+#39+
                           ' AND   SEQ_CED ='+#39+copy(cxDBMaskEdit1.Text,5,7)+#39+
                           ' AND   VER_CED ='+#39+copy(cxDBMaskEdit1.Text,13,1)+#39;
         qfind.Open;
         If (qfind.RecordCount =0) Then
         begin
