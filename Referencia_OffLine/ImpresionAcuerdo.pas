    ...
    begin
      ...
      begin
          ...
          SQL.Text := ' SELECT C.CLIENTEID FROM PTCLIENTE C INNER JOIN PRICEDISCGROUP P ON C.CLIENTEID=P.CLIENTEID WHERE GRUPOPRECIO='+#39+CodCliente+#39+
                      ' AND GRUPOCLIENTE='+#39+'01'+#39+' AND TIENE_CUPS='+#39+'S'#39;
          Open;
      ...
      begin
          Seleccion :=  ' SELECT DISTINCT PRUEBARELATION AS PRUEBA,P.DESCRIPCION,'+
                        ' ISNULL((SELECT CODIGOCUPID FROM PTCODIGOCUP WHERE PRUEBAID=A.PRUEBARELATION AND CLIENTEID='+#39+Resultado+#39+'),'+#39+#39+') AS CODIGO_CUP '+
                        ' ,DESDE,HASTA,A.PRECIO FROM PTACUERDOCOMERCIAL A INNER JOIN PTPRUEBA P '+
                        ' ON A.PRUEBARELATION=P.PRUEBAID LEFT JOIN PTCODIGOCUP C ON C.PRUEBAID=A.PRUEBARELATION '+
