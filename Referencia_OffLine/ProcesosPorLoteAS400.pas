...
type
  TfrmProcesosPorLoteAS400 = class(TForm)
    ...
    qrEntradaPacienteESTATUS_SAP: TStringField;
...
begin
  if Not dm.PingHost('172.16.30.100',5) then
  begin
     ShowMessage('Servidor de As400 aún no está disponible.');
  end;
end;
...
{    if Not dm.PingHost('10.0.0.1',5) then
    ...
    begin
       ShowMessage('Fecha de Corte ó Sucursal NO pueden estar en Blanco.');
       Exit;
    end;
...
//          ' Fecha between '+#39+'20170701'+#39+' and  '+#39+'20170704'+#39+
//          ' Pacienteid in ('+#39+'00008805'+#39+','+#39+'00018822'+#39+','+#39+'00029127'+#39+','+#39+'00029448'+#39+') And Estatus='+#39+'0'+#39+' And DataAreaID='+#39+DM.CurEmpresa+#39+
          ' MUESTRANO IN (SELECT NUMERO1 FROM [DBFIN].[S10D7471].INTERFASE.LAT005PEN)'+
          ' ORDER BY SucursalId,Muestrano ';
//          ' AND MUESTRANO ='+#39+'17027013410'+#39+'ORDER BY SucursalId,Muestrano ';
{
    ...
    begin
         ...
         Begin
            ...
            qrEntradapacienteDetalle.sql.Text := ' SELECT * FROM ptEntradapacienteDetalle (nolock) Where Refrecid='+FieldByName('RecID').AsString+
 ...
 {                        If ((qrEntradaPacientePublicarInternet.Value = 1) OR (qrEntradaPacientePublicarInternet.Value = 0))
                         ...
                         Begin
                           ...
                           Begin
                                if dm.PingHost('172.16.30.100',1) then
                                begin
                                   Buscar_Paciente;
            ...
            //             Generar_Labels;
                      ...
                      begin
                         Try
                           ...
                           InterfaseAS400.MandarPruebaAS4000(qrEntradaPacienteMuestraNo.Value, qrEntradaPacientePacienteId.Value);
                           InterfaseAS400.ASConnection.Close;
                         Except
    ...
    on E : Exception do
      ShowMessage(E.ClassName+' error pasando Entrada='+qrEntradaPacienteMuestrano.Value+' con mensaje : '+E.Message);
 end;
  DM.Info('Fin Proceso Actualización por Lotes al AS400.');
end;
...
//                  InterfaseAS400.CambiarTelefonoAS400(FormatFloat('00000000000',InterfaseAS400.qrASBADPHONEIN6NUM.Value));
      ...
      Except
        Raise exception.CreateFmt(' Existe un Problema de Comunicacion Con el Servidor AS-400. '+ #13 +
                                  ' Por Favor, LLame al Departamento de Computos. '+ #13 +
                                  ' ó Intentelo Nuevamente.!!!'+'  '+FormatFloat('00000000000',InterfaseAS400.qrASBADPHONEIN6NUM.Value), []);
      End;
    End;
end;
