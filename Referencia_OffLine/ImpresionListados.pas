...
type
  TfrmImpresionListados = class(TForm)
...
var
  frmImpresionListados: TfrmImpresionListados;
  Seleccion : string;
...
procedure TfrmImpresionListados.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if (key = Vk_Down) then
...
procedure TfrmImpresionListados.Run;
begin
 showmodal;
 if ModalResult = mrOk then
 begin
  if MessageDlg('Desea Imprimir el Listado de Empleados?', mtWarning, [mbYes,mbNo], 0) = mrYes then
  begin
    ...
    If (CbOrdenar.ItemIndex = 0) Then
      Seleccion := Seleccion + ' ORDER BY Cargo, MuestraId ';
    If (CbOrdenar.ItemIndex = 1) Then
      Seleccion := Seleccion + ' ORDER BY Departamento, MuestraId ';
    If (CbOrdenar.ItemIndex = 2) Then
      Seleccion := Seleccion + ' ORDER BY EmpleadoID, MuestraId ';
    If (CbOrdenar.ItemIndex = 3) Then
      Seleccion := Seleccion + ' ORDER BY EmpresaID, MuestraId ';
    If (CbOrdenar.ItemIndex = 4) Then
      Seleccion := Seleccion + ' ORDER BY MuestraId ';
    If (CbOrdenar.ItemIndex = 5) Then
      Seleccion := Seleccion + ' ORDER BY Sucursal, MuestraId ';
   ...
   If (FrmMain.frmPlanCliente.qrClienteReporte.RecordCount > 0) Then
     Begin
       FrmMain.frmPlanCliente.ppClientesPlanesListado.DeviceType := dtScreen;
       FrmMain.frmPlanCliente.ppClientesPlanesListado.Print;
     End
   Else
