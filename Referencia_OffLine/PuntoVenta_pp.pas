...
uses
  ...
  cxDBEditRepository, cxExtEditRepositoryItems, cxEditRepositoryItems, WebservicePalic,
...
type
  TfrmPuntoVenta = class(TfrmCustomModule)
    ...
    procedure ppImpCotizacionBeforePrint(Sender: TObject);
...
begin
  ...
  if qrEntradaPacienteClienteID.Value <> '' then
   begin
    qfind := DM.Find_LDR('SELECT * FROM PTCliente (nolock) WHERE ClienteId = :cli', qrEntradaPacienteClienteID.Value);
...
Result := False;
Try
  with  dm.sp_ActPagos, Parameters do
  begin
     ...
     While not qrEntradaPacienteDetalle.Eof Do
      Begin
        If (Trim(qrEntradaPacienteDetalleCOMENTARIO.Value) = '') Or
          ...
          Begin
            If (qrEntradaPacienteDetallePruebaId.Value <> '00003978') And
               (qrEntradaPacienteDetallePruebaId.Value <> '00003981') And
               (qrEntradaPacienteDetallePruebaId.Value <> '00003982') Then
              Begin
                qrEntradaPacienteDetalle.EnableControls;
//                qrEntradaPacienteDetalle.First;
...
begin
  ...
  qpruebadesc := DM.Find_Ldr('SELECT * FROM dbo.PtPrueba (nolock) WHERE PruebaID = :pru', qrEntradaPacienteDetallePruebaID.value);
...
begin
  qprint  := DM.NewQuery;
  qprint.Close;
  qprint.SQL.Text := ' Exec dbo.Lista_Labels @Numero_Mue='+#39+_Muestrano+#39;
  qprint.Open;
...
begin
  ...
  begin
      ...
      dm.qrNCF.SQL.Text := ' Select Convert(Varchar(50),CASE NCFTYPEID  WHEN '+#39+'01'+#39+' THEN '+#39+'Factura Crédito Fiscal'+#39+
                           ...
                           ' END) As NCFTYPEID,Muestrano,EntradaID,NCFID,NCFNAME,RNC,Fecha From AX2009.dbo.NCFTABLEPTV (nolock) '+
...
//      vURLWSDL     := 'http://10.0.0.30/wsreferencia/referenciawebservice.asmx';
  Try
      ...
      qAutoriza.Close;
      qAutoriza.SQL.Text := ' SELECT distinct CodProveedor,dbo.rellena(a.CodProveedor,5) as CodProvUniv,a.WsArsid,p.Url,p.Password,p.Usuario,a.SucEx,a.proveedor,a.Monto_Limite from dbo.ptars a (nolock) ,ptcliente c (nolock), dbo.ptProveedor p (nolock)'+
                            ' where a.CodProveedor=c.CarnetNumero and a.proveedor=p.proveedorid'+
                            ' and c.Clienteid='+#39+qrEntradaPacienteClienteID.Asstring+#39;
      qAutoriza.Open;
      ...
      Begin
          ...
          case Dm.codArs of
             1  :Begin
                    ...
                    if ((CAfiliado.TipoPlan='B') and (Trim(qrEntradapacienteClienteID.Value)<>'00000968')) Or
                       ((CAfiliado.TipoPlan='C') and (Trim(qrEntradapacienteClienteID.Value)<>'00000137')) then
                    Begin
                        Screen.Cursor     := crDefault;
                        spMensaje.Visible := False;
             ...
             2  :Begin
                    ...
                    if ((CAfiliado.TipoPlan='B') and (Trim(qrEntradapacienteClienteID.Value)<>'00000967')) Or
                       ((CAfiliado.TipoPlan='C') and Not((Trim(qrEntradapacienteClienteID.Value)='00000200') Or (Trim(qrEntradapacienteClienteID.Value)='00033217'))) then
                    Begin
                        Screen.Cursor     := crDefault;
                        spMensaje.Visible := False;
                        EtMensajeDlg('Afiliado NO pertenece a este tipo de Plan de Seguro.  Verifique.', etAviso, [etOk],0, dm.Imagenes.Items[4].Bitmap,true);
      ...
      Begin
          ...
          begin
            ...
            vArrayOfRecdatben2ObjUser := WSUniv.Validacion_de_Contrato(Dm.CurUserUniv, ifThen(qrEntradaPacienteClienteId.Value='00000964','01','03'),StrContrato ,dfamilia,'1980-01-01T00:00:00', CodProvUniv, 1,qrEntradaPacienteDetalle.RecordCount);
          end
          else
          begin
            dfamilia := 0;
            vArrayOfRecdatben2ObjUser := WSUniv.Validacion_de_Contrato(Dm.CurUserUniv, ifThen(qrEntradaPacienteClienteId.Value='00000964','01','03'),qrEntradaPacientePolizaID.Value ,dfamilia,'1980-01-01T00:00:00', CodProvUniv, 1,qrEntradaPacienteDetalle.RecordCount);
          end;
...
//          WSPalic.fValidaPassword('REFERENCIA','REF5545');
          ...
          if Not(((strTipo_PlanAf='B') and (Trim(qrEntradapacienteClienteID.Value)='00000965')) Or
             ((strTipo_PlanAf='C') and (Trim(qrEntradapacienteClienteID.Value)='00000134')) Or
             ((strTipo_PlanAf='A') and ((Trim(qrEntradapacienteClienteID.Value)='00000134') Or (Trim(qrEntradapacienteClienteID.Value)='00000965'))) Or
             (Trim(qrEntradapacienteClienteID.Value)='00033218')) Then
          Begin
              Screen.Cursor     := crDefault;
              spMensaje.Visible := False;
              EtMensajeDlg('Afiliado NO pertenece a este tipo de Plan de Seguro.  Verifique.', etAviso, [etOk],0, dm.Imagenes.Items[4].Bitmap,true);
          ...
          Dm.Afiliado     := strNombreAfil;
          Dm.TipoPlan     := strTipo_PlanAf;
          Dm.Num_Session  := strNum_Session;
          frmMain.LanzaVentana(-8034);
          If btgrabar.Enabled = True then btgrabar.SetFocus;
      end
      else
      ...
      Begin
          ...
          if (Trim(qrEntradapacienteClienteID.Value)<>'00001012') then
          Begin
              Screen.Cursor     := crDefault;
              spMensaje.Visible := False;
              EtMensajeDlg('Afiliado NO pertenece a este tipo de Plan de Seguro.  Verifique.', etAviso, [etOk],0, dm.Imagenes.Items[4].Bitmap,true);
  ...
  inherited;
   ...
   begin
     ...
     begin
        ...
        qAutoriza := DM.NewQuery;
        qAutoriza.Close;
        qAutoriza.SQL.Text := ' SELECT distinct CodProveedor,dbo.rellena(a.CodProveedor,5) as CodProvUniv,a.WsArsid,p.Url,p.Password,p.Usuario,a.SucEx from dbo.ptars a (nolock),ptcliente c (nolock), dbo.ptProveedor p (nolock)'+
                            ' where a.CodProveedor=c.CarnetNumero and a.proveedor=p.proveedorid'+
                            ' and c.Clienteid='+#39+qrEntradaPacienteClienteID.Asstring+#39;
        qAutoriza.Open;
        if (qAutoriza.RecordCount > 0) then
...
//     qrEntradaPacienteUserId.Value := DM.CurUser;
     ...
     If (qrEntradaPacienteOrigen.Value = DM.qrParametroGrupoExt.Value) Or
        ...
        (qrEntradaPacienteFactExterior.Value = 1) Then
     begin
           If ((dm.TotalPruebas_USA) = 1) And (Not (qrEntradaPacienteDetalle.Locate('PruebaID','00003978',[]))) then
           begin
           ...
           If ((dm.TotalPruebas_USA) = 2) And (Not (qrEntradaPacienteDetalle.Locate('PruebaID','00003981',[]))) then
           begin
              if dm.IsAdicionalUSA = True then
              begin
                If MessageDlg('Por favor registre el cargo adicional de esta orden.  Desea registrarlo?', mtConfirmation, [mbYes,mbNo], 0) = mrYes then
           ...
           If ((dm.TotalPruebas_USA) >= 3) And (Not (qrEntradaPacienteDetalle.Locate('PruebaID','00003982',[]))) then
           begin
              if dm.IsAdicionalUSA = True then
              begin
                If MessageDlg('Por favor registre el cargo adicional de esta orden.  Desea registrarlo?', mtConfirmation, [mbYes,mbNo], 0) = mrYes then
...
{  if Copy( mdBeneficiarioCNombres.AsString,1,3) <> Copy(Trim(qrEntradapacienteNombrePaciente.Value),1,3) then
  ...
  if ((Dm.TipoPlan='PBS') and (Trim(qrEntradapacienteClienteID.Value)<>'00000964')) Or
     ((Dm.TipoPlan<>'PBS') and (Trim(qrEntradapacienteClienteID.Value)='00000964')) then
  Begin
      Screen.Cursor     := crDefault;
      spMensaje.Visible := False;
      EtMensajeDlg('Afiliado NO pertenece a este tipo de Plan de Seguro.  Verifique.', etAviso, [etOk],0, dm.Imagenes.Items[4].Bitmap,true);
...
begin
  ...
  qfindSucursal := DM.Find_Ldr('SELECT * FROM PTSucursal (nolock) WHERE SucursalId = :suc', DM.CurSucursal);
...
//               qrEntradaPacienteDetallePASARAXAPTA.Value := 0; //TRANSFERIR A AXAPTA (False).
               ...
               qrEntradaPacienteDetallePrecio.Value := DM.Precio_Prueba_Configurado(qrEntradaPacienteDetallePruebaID.Value,
                                                                                    ...
                                                                                    qrEntradaPacienteOrigen.value,dm.GroupPrivado,GrupoPrecio,qrEntradaPacienteClaseCredito.Value) * factor;
...
begin
  ...
  Begin
    qrEntradaPacienteDetalle.Edit;
    qfindprueba := DM.Find_Ldr('Select * From dbo.PtPrueba (nolock) WHERE PruebaID = :pru',qrEntradaPacienteDetallePruebaID.value);
    ...
    If (qrEntradaPacienteOrigen.Value = DM.qrParametroGrupoExt.Value) Or
      ...
      Begin
        ...
        qfindRepetir.Close;
        qfindRepetir.SQL.Text := ' Select * From dbo.PtPrueba (nolock) '+
                                 ' Where Exterior = 1 '+
                                 ' AND DataAreaId = ' + #39 + DM.CurEmpresa + #39 +
                                 ' AND PruebaID = :pru ';
    ...
    begin
      ...
      begin
          ...
          dm.qrNCF.SQL.Text := ' Select CASE NCFTYPEID  WHEN '+#39+'01'+#39+' THEN '+#39+'Factura Crédito Fiscal'+#39+
                               ...
                               ' WHEN '+#39+'14'+#39+' THEN '+#39+'Factura Régimen Especial'+#39+
                               ' WHEN '+#39+'15'+#39+' THEN '+#39+'Factura Régimen Gubernamental'+#39+
                               ' END NCFTYPEID,Muestrano,EntradaID,NCFID,NCFNAME,RNC,Fecha From AX2009.dbo.NCFTABLEPTV (nolock) '+
                               ' Where muestrano='+#39+qrEntradaPacienteMuestrano.value+#39+' And EntradaID='+#39+qrEntradaPacienteEntradaid.value+#39+' And DataAreaID='+#39+DM.CurEmpresa+#39;
          dm.qrNCF.Open;
...
//   qfind := DM.Find_Ldr('SELECT * FROM PTCliente WHERE ClienteId = :cli', qrEntradaPacientePacienteId.Value);
   qfind := DM.Find_Ldr('SELECT * FROM PTCliente (nolock) WHERE ClienteId = :cli', qrEntradaPacientePacienteId.Value);
...
var
 qfind : TADOQuery;
begin
 qfind := DM.Find('SELECT * FROM SysUsuarios (nolock) WHERE UserID = :user',DM.CurUser);
...
//  Begin
    ...
    qfindCltParams := DM.Find_Ldr('SELECT * FROM PTCliente (nolock) WHERE ClienteID = :cli', qrEntradaPacienteClienteId.Value);
            ...
            begin
              ...
              begin
               ...
               begin
                   ...
                   begin
                     InterfaseAS400.Buscar_Cedula_Internet(qfindPacientes.FieldByName('Identificacion').AsString,qrEntradaPacienteMuestrano.AsString,qfindPacientes.FieldByName('Tipo_Documento').AsString);
                     ClienteID:=dm.Retorna_Cliente_Internet(qfindPacientes.FieldByName('Identificacion').AsString);
              ...
              begin
                  ...
                  DM.qrInternet.SQL.Text := ' SELECT top 1 * FROM ptUsuariosInternet (nolock) '+
                                            ' WHERE Tipo = '+#39+'P'+#39+
                                            ' AND Substring(Replace(Identificacion,'+#39+'-'+#39+','+#39+''+#39+'),1,11)=Substring(REPLACE('+#39+dm.Busca_Internet_Identificacion(qfindPacientes.FieldByName('Identificacion').AsString)+#39+','+#39+'-'+#39+','+#39+''+#39+'),1,11)'+
                                            ' AND DataAreaId = ' + #39 + DM.CurEmpresa + #39;
                  DM.qrInternet.Open;
...
procedure TfrmPuntoVenta.ppCabeFactSeguroBeforePrint(Sender: TObject);
begin
...
procedure TfrmPuntoVenta.ppImpCotizacionBeforePrint(Sender: TObject);
begin
  inherited;
   if qrEntradaPacienteSexo.value = 1 then
      ppLabel42.Caption := 'Masculino'
   else if qrEntradaPacienteSexo.value = 0 then
      ppLabel42.Caption := 'Femenino';
end;
...
//  qfindprint := DM.find('Select * from PTPrueba where PruebaID = :pru', qrEntradaPacienteDetalleCodigoCupID.Value);
  qfindprint := DM.Find_Ldr('SELECT * FROM dbo.PtPrueba (nolock) where PruebaID = :pru', qrEntradaPacienteDetallePruebaId.Value);
...
procedure TfrmPuntoVenta.ppDetailBand8BeforePrint(Sender: TObject);
begin
 ppLabel262.Caption :=qrEntradaPacienteDetallePruebaId.Value;
 ppDBComentario.Visible := True;
 ppDBDescripcion.Visible := False;
end;
...
begin
 ...
 qtest := DM.Find_Ldr('SELECT * FROM dbo.PtPrueba (nolock) WHERE PruebaID = :pru', prueba);
 ...
 While Not qrEntradaPacienteDetalle.Eof Do
  Begin
   If Not Validar_Label_Temp(qrEntradaPacientePacienteId.Value,
     ...
     Begin
       qfindprulab := DM.Find_Ldr('SELECT * FROM dbo.PtPrueba (nolock) WHERE PruebaID = :pru', qrEntradaPacienteDetallePruebaID.Value);
...
begin
  ...
  if (ClienteFirst <> '') And (ClienteSecond <> '') And (ClienteFirst <> ClienteSecond) Then
    Begin
      qfind1 := DM.Find_Ldr('SELECT * FROM PTCliente (nolock) WHERE ClienteId = :cli', ClienteFirst);
      qfind2 := DM.Find_Ldr('SELECT * FROM PTCliente (nolock) WHERE ClienteId = :cli', ClienteSecond);
...
begin
  ...
  While Not qrEntradaPacienteDetalle.Eof Do
   Begin
     ...
     //Calculando el Descuento a las PRUEBAS y los COMBOS.
     qprueba := DM.Find_Ldr('SELECT * FROM dbo.PtPrueba (nolock) WHERE PruebaID = :pru', qrEntradaPacienteDetallePruebaID.value);
           ...
           Begin
             If (comboprecio = True) Then
               Begin
                 ...
                 else if Precio_Cabecera = 1 then
                 begin
                    qrEntradaPacienteDetallePrecio.Value := 0;
                 end;
                 //Validaciones de las Pruebas de Seguro.
...
Var
 sqlString : String;
 qfind, qfindciudad,qfindseg : TADOQuery;
begin
// If ( qrEntradaPaciente.State in [dsinsert,dsedit] ) Then
 If ( (qrEntradaPaciente.State in [dsinsert, dsedit]) ) And
    ( (qrEntradaPacienteClienteID.Value = '') Or
  ...
  Begin
   ...
   If (DM.qrPacientes.RecordCount > 0) Then
    Begin
      ...
      qfindciudad := DM.Find_Ldr('SELECT * FROM PTCIUDAD (nolock) Where CiudadID = :ciu', DM.qrPacientesCiudadID.AsString);
      qfindseg    := DM.Find_Ldr('SELECT * FROM PTCLIENTE (nolock) Where Estatus = 0 And ClienteID = :clt', DM.qrPacientesSeguro.Value);
...
begin
 ...
 begin
    ...
    strsql := strsql + ' AND SUBSTRING(PRUEBAID,5,4) IN ( SELECT SUBSTRING(COD_COMEN,3,4) FROM '+
                       ' dbo.PTCOMENTARIOPRUEBAS (nolock) WHERE ESTATUS='+#39+'A'+#39+' AND SUBSTRING(COD_COMEN,1,2)='+#39+'CU'+#39+') AND ';
 end
 else
 begin
                  ...
                  Begin
                   ...
                   If (qentrada.Recordcount > 0) then
                     Begin
                       ...
                       qrEntradaPacienteDetalle.Open;
                       qSucursal := DM.Find_Ldr('SELECT * FROM PTSucursal (nolock) WHERE Sucursal = :suc', InterfaseAS400.qrPagosAs400IN14SU.AsString);
...
begin
 ...
 begin
  ...
  Add(' Case When (datediff(yy,'+#39+FormatDateTime('yyyymmdd',qrEntradapacienteFechaNacimiento.Value)+#39+','+#39+FormatDateTime('yyyymmdd',DM.SystemDate)+#39+') - case when '+#39+FormatDateTime('yyyymmdd',DM.SystemDate)+#39+ '< dateadd(yy,datediff(yy,'+#39+FormatDateTime('yyyymmdd',qrEntradapacienteFechaNacimiento.Value)+#39+','+#39+FormatDateTime('yyyymmdd',DM.SystemDate)+#39+'),'+#39+FormatDateTime('yyyymmdd',qrEntradapacienteFechaNacimiento.Value)+#39+') then 1 else 0 end) < 1 and ');
  Add(' (datediff(mm,'+#39+FormatDateTime('yyyymmdd',qrEntradapacienteFechaNacimiento.Value)+#39+','+#39+FormatDateTime('yyyymmdd',DM.SystemDate)+#39+') - case when '+#39+FormatDateTime('yyyymmdd',DM.SystemDate)+#39+ '< dateadd(mm,datediff(yy,'+#39+FormatDateTime('yyyymmdd',qrEntradapacienteFechaNacimiento.Value)+#39+','+#39+FormatDateTime('yyyymmdd',DM.SystemDate)+#39+'),'+#39+FormatDateTime('yyyymmdd',qrEntradapacienteFechaNacimiento.Value)+#39+') then 1 else 0 end) > 0  ');
  Add(' then cast(datediff(mm,isnull('+#39+FormatDateTime('yyyymmdd',qrEntradapacienteFechaNacimiento.Value)+#39+',0), '+#39+FormatDateTime('yyyymmdd',DM.SystemDate)+#39+') as varchar(10))+'+#39+' Mes(es)'+#39);
  Add('      When (datediff(yy,'+#39+FormatDateTime('yyyymmdd',qrEntradapacienteFechaNacimiento.Value)+#39+','+#39+FormatDateTime('yyyymmdd',DM.SystemDate)+#39+') - case when '+#39+FormatDateTime('yyyymmdd',DM.SystemDate)+#39+' < dateadd(yy,datediff(yy,'+#39+FormatDateTime('yyyymmdd',qrEntradapacienteFechaNacimiento.Value)+#39+','+#39+FormatDateTime('yyyymmdd',DM.SystemDate)+#39+'),'+#39+FormatDateTime('yyyymmdd',qrEntradapacienteFechaNacimiento.Value)+#39+') then 1 else 0 end) < 1 and ');
  Add(' (datediff(mm,'+#39+FormatDateTime('yyyymmdd',qrEntradapacienteFechaNacimiento.Value)+#39+','+#39+FormatDateTime('yyyymmdd',DM.SystemDate)+#39+') - case when '+#39+FormatDateTime('yyyymmdd',DM.SystemDate)+#39+ '< dateadd(mm,datediff(yy,'+#39+FormatDateTime('yyyymmdd',qrEntradapacienteFechaNacimiento.Value)+#39+','+#39+FormatDateTime('yyyymmdd',DM.SystemDate)+#39+'),'+#39+FormatDateTime('yyyymmdd',qrEntradapacienteFechaNacimiento.Value)+#39+') then 1 else 0 end) = 0  ');
  Add(' then cast(datediff(dd,isnull('+#39+FormatDateTime('yyyymmdd',qrEntradapacienteFechaNacimiento.Value)+#39+',0), '+#39+FormatDateTime('yyyymmdd',DM.SystemDate)+#39+') as varchar(10))+'+#39+' Dia(s)'+#39);
  Add('      When (datediff(yy,'+#39+FormatDateTime('yyyymmdd',qrEntradapacienteFechaNacimiento.Value)+#39+','+#39+FormatDateTime('yyyymmdd',DM.SystemDate)+#39+') - case when '+#39+FormatDateTime('yyyymmdd',DM.SystemDate)+#39+' < dateadd(yy,datediff(yy,'+#39+FormatDateTime('yyyymmdd',qrEntradapacienteFechaNacimiento.Value)+#39+','+#39+FormatDateTime('yyyymmdd',DM.SystemDate)+#39+'),'+#39+FormatDateTime('yyyymmdd',qrEntradapacienteFechaNacimiento.Value)+#39+') then 1 else 0 end) > 0 then ');
  Add(' cast((datediff(yy,'+#39+FormatDateTime('yyyymmdd',qrEntradapacienteFechaNacimiento.Value)+#39+','+#39+FormatDateTime('yyyymmdd',DM.SystemDate)+#39+') - case when '+#39+FormatDateTime('yyyymmdd',DM.SystemDate)+#39+'< dateadd(yy,datediff(yy,'+#39+FormatDateTime('yyyymmdd',qrEntradapacienteFechaNacimiento.Value)+#39+','+#39+FormatDateTime('yyyymmdd',DM.SystemDate)+#39+'),'+#39+FormatDateTime('yyyymmdd',qrEntradapacienteFechaNacimiento.Value)+#39+') then 1 else 0 end) as varchar(10))+'+#39+' Año(s)'+#39);
  ...
  Add(' IsNull(( SELECT WsArsid from dbo.ptars a (nolock) ,ptcliente c (nolock)  where a.CodProveedor=c.CarnetNumero  and c.Clienteid='+#39+qrEntradaPacienteClienteID.value+#39+'),'+#39+'0'+#39+') As WsArsID, ');
  Add(' IsNull(( SELECT Descripcion from dbo.PTDiagnostico (nolock) Where cod_diag='+#39+Trim(qrEntradaPacienteCOD_DIAG.value)+#39+' and Cod_Ars in ( SELECT WsArsid from dbo.ptars a (nolock) ,ptcliente c (nolock)  where a.CodProveedor=c.CarnetNumero  and c.Clienteid='+#39+qrEntradaPacienteClienteID.value+#39+')'+'),'+#39+''+#39+') As Desc_Diag ');
  ...
  begin
         ...
         Begin
           ...
           ppLabel220.caption := fieldbyname('Cod_Prov_Ref').value;
...
//           pp2DBarCode1.Text  := qrEntradaPacienteMuestrano.Value;
         ...
         Begin
           ppLabel240.Caption := fieldbyname('Nombre_Sucursal').value;
...
begin
 ...
 Begin
    ...
    strsql := 'Select PruebaID From dbo.ptprueba (nolock) '+
...
begin
 ...
 strPruebaAnt := '';
 qfind.Close;
 strsql := 'Select Top 1 Cod_Sec_Prueba From dbo.ptprueba (nolock) '+
